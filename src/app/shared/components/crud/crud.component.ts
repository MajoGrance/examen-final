...
export class CRUDComponent implements OnInit, AfterViewInit {
        ...
        import("xlsx").then(xlsx => {
            ...
            const exportData = sourceCopy.map((obj: any) => {
                for (const col of Object.keys(obj)) {
                    const colSettings = this.columns.find(colSet =>  colSet.field === col || colSet.refName === col);
                    if (colSettings?.refField) {
                        const value = obj[col];
                        const newValue = value?value[colSettings.refField]:'';
                        obj[col] = newValue;
                    }
                }
                return obj;
    ...
    async deleteSelected(): Promise<void> {
        ...
        for(const select of this.selected) {
            const resp = await this.service.delete(select.id);
            if (resp.ok) {
                this.toastService.show('top-right', 'success', resp.msg, select.id);
            } else {
                this.toastService.show('top-right', 'error', resp.msg, resp.resp);
            }
        }
    ...
    async delete(row: any): Promise<void> {
        ...
        const resp = await this.service.delete(row.id);
        if (resp.ok) {
            this.toastService.show('top-right', 'success', resp.msg, row.id);
        } else {
            this.toastService.show('top-right', 'error', resp.msg, resp.resp);
        }
        this.selected = [];
